...
* getEventDefaults - (function) Return the default object to be merged into for all events. By default, contains things like page url, user agent, time, etc.
...
MooTune Method: identify
------------------------

A method to use the identify functionality found in many popular analytics backends. Uses the
`identify` method found on the backend. By default, gets a unique id using the method
`getIdentity` found in the options, which stores an id generated by `generateId` in a 100-day cookie.
...
1. id - optional id to override this.options.generateId
